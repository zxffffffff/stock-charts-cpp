...
std::tuple<bool, NumberCore> FunctionParser::MA(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
    ...
    if (period <= 1) {
        ...
        if (prev_calculated == 0) {
            ...
            for (int i = 0; i < start_position - 1; i++)
                buffer[i] = NumberCore::EmptyValue;
...
std::tuple<bool, NumberCore> FunctionParser::EMA(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::SMA(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 3)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::LLV(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::HHV(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::REF(const std::vector<NumberCore>& inputs)
{
    if (inputs.size() != 2)
        return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::REFX(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::STD(const std::vector<NumberCore>& inputs)
{
	//zxf todo
	return STDP(inputs);
}

std::tuple<bool, NumberCore> FunctionParser::STDP(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::processMAX(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() < 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::processMIN(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() < 2)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::processABS(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 1)
		return emptyRet();
	...
	for (int i = 0; i < buffer.size(); ++i) {
		...
		if (d != NumberCore::EmptyValue)
			buffer[i] = std::abs(d);
	}
    return { true, buffer };
}

std::tuple<bool, NumberCore> FunctionParser::processSUM(const std::vector<NumberCore>& inputs)
{
    if (inputs.size() != 2)
        return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::BARSLASTCOUNT(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 1)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::BETWEEN(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 3)
		return emptyRet();
...
std::tuple<bool, NumberCore> FunctionParser::BACKSET(const std::vector<NumberCore>& inputs)
{
	if (inputs.size() != 2)
		return emptyRet();
