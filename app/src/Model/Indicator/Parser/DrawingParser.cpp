...
std::tuple<bool, ExpDrawing, NumberCore> DrawingParser::process(const std::string& name, const std::vector<NumberCore>& inputs)
{
...
std::tuple<bool, ExpDrawing, NumberCore> DrawingParser::number(const std::vector<NumberCore>& inputs)
{
	...
	for (int i = 0; i < rates_total; ++i) {
		...
		if (b == 0.0 || b == NumberCore::EmptyValue)
			continue;
		buffer[i] = price[i];
...
std::tuple<bool, ExpDrawing, NumberCore> DrawingParser::stickLine(const std::vector<NumberCore>& inputs)
{
	...
	if (cond.isEmpty() || price.isEmpty() || price2.isEmpty() || width == NumberCore::EmptyValue || empty == NumberCore::EmptyValue)
		return emptyRet(true);
