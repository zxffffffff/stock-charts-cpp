...
void IndicatorParser::setFormula(const IndexFormula& formula)
{
	m_formula = formula;
}

void IndicatorParser::setStockCore(const StockCore& p)
{
...
void IndicatorParser::setStockExt(const StockRelyData& p)
{
	setStockExt(std::make_shared<StockRelyData>(p));
}

void IndicatorParser::setStockExt(std::shared_ptr<StockRelyData> p)
{
...
bool IndicatorParser::run()
{
	m_result = ResultIndex();
	m_stockRely.clear();
	m_bParseError = false;
	m_errWord.clear();
	...
	for (i = 0; i < expressions.size(); ++i) {
		...
		m_dValue = NumberCore::EmptyValue;
		m_sValue.clear();
		m_expInfo = ExpInfo();
...
const ResultIndex& IndicatorParser::getResult()
{
	return m_result;
}

const std::set<EnStockRely>& IndicatorParser::getStockRely()
{
	return m_stockRely;
}

const IndexFormula& IndicatorParser::getFormula()
{
	return m_formula;
}

NumberCore IndicatorParser::parseFormula()
{
...
NumberCore IndicatorParser::parseRelationOperate()
{
...
NumberCore IndicatorParser::parseArithLastOperate()
{
...
NumberCore IndicatorParser::parseArithFirstOperate()
{
...
NumberCore IndicatorParser::parsePrimTokenOperate()
{
	...
	case EnParseToken::String:
		...
		if (m_eToken == EnParseToken::Rename || m_eToken == EnParseToken::RenameAssign) {
			...
			if (rely != EnStockRely::None)
				m_stockRely.insert(rely);
...
EnParseToken IndicatorParser::parseTokenValue()
{
	switch (*m_iteCurrent)
	{
	case ';':
...
Number IndicatorParser::parseNumberValue()
{
	std::stringstream ss;
